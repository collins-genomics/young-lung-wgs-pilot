...
workflow SAIGE_GENE_EUROPEAN {
  input {
    ...
    File groupfile_singleton = "gs://fc-73a5dbdd-51d6-4c28-a211-234c026bdb26/step_9_run_vep/STEP_9_RUN_VEP/v2/ufc_singleton.groupfile"
  ...
  call saige_gene_step2 as saige_gene_step2_singleton{
    input:
      pathogenic_criteria = "all",
      ...
      group_file = groupfile_singleton,
      varianceRatio = saige_gene_step1.varianceRatio,
      sparseGRMFile = saige_gene_step0.sparseGRMFile,
      sparseGRMSampleIDFile = saige_gene_step0.sparseGRMSampleIDFile
  }
  call saige_gene_step2_beta as saige_gene_step2_beta_singleton{
    input:
      pathogenic_criteria = "all",
      vcf = ConcatVcfs.merged_vcf,
      vcf_idx = ConcatVcfs.merged_vcf_idx,
  ...
  call sortSAIGE_Output {
    input:
      saige_output_001 = saige_gene_step2_singleton.out1,
      saige_output_beta_001 = saige_gene_step2_beta_singleton.out1,
...
task get_covariates {
  ...
  # Write to file
  covariates_data.to_csv("yl_covariates.tsv",sep='\t', index=False)
  covariates_data['sample_id'].to_csv("yl_subjects.list", index=False, header=False)
  print("File written to yl_covariates.tsv")
  CODE
  >>>
  runtime {
    docker: "vanallenlab/pydata_stack"
  ...
  output {
    File covariate_data = "yl_covariates.tsv"
    File subjects_list = "yl_subjects.list"
  }
}
...
task saige_gene_step1 {
  ...
  step1_fitNULLGLMM.R \
      ...
      --traitType=binary \
      --covarColList=sex_binary,PC1_EUR,PC2_EUR,PC3_EUR,PC4_EUR,PC5_EUR \
